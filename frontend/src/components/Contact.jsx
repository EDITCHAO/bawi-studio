...
const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!formData.domain) {
        throw new Error('Veuillez sélectionner un domaine');
      }

      if (formData.domain === 'other' && !formData.customDomain) {
        throw new Error('Veuillez préciser votre domaine d\'activité');
      }

      if (!formData.projectType) {
        throw new Error('Veuillez sélectionner un type de projet');
      }

      // Validation budget et deadline seulement si ce n'est pas un étudiant
      if (!isStudent) {
        if (!formData.budget) {
          throw new Error('Veuillez sélectionner un budget');
        }
        if (!formData.deadline) {
          throw new Error('Veuillez sélectionner une deadline');
        }
      }
      ...
      const submitData = {
        ...
        customDomain: formData.domain === 'other' ? formData.customDomain : null,
